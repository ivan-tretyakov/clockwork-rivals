...
try {
  ...
  for (const objective of hostOffers)
    assert.equal(JSON.stringify(guest.snapshot).includes(objective), false);
  for (const objective of guestOffers)
    assert.equal(JSON.stringify(host.snapshot).includes(objective), false);
  ...
  if (first.action.type === "choose-objective") {
    assert.equal(
      active.snapshot!.state.objective!.selected,
      first.action.objective,
    );
    const opponent = firstSeat === "P0" ? guest : host;
    assert.equal(
      JSON.stringify(opponent.snapshot).includes(first.action.objective),
      false,
    );
  }
  assert.equal((await send(active, first)).duplicate, true);
